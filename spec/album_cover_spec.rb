RSpec.describe 'HTML Album Cover' do
    ...
    context 'within <body>' do
      it 'the title-box section contains the correct header sizes and text content' do
        ...
        expect(title_box.children.select {|ch| ch.name == "h4"}[1].children.first.text).to include("FROM <strong>DESPICABLE ME"), "A second 'h4' tag should contain the text 'FROM DESPICABLE ME'"
